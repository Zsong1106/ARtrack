...
class BaseBackbone(nn.Module):
    ...
    def forward(self, z, x, identity, **kwargs):
        ...
        cx1, cx2, cx3, cx4 = self.spm(x)
        c2, c3, c4 = self._add_level_embed(cx2, cx3, cx4)
        cx = torch.cat([c2, c3, c4], dim=1)  # B*N*C
        #print('cx.shape', cx.shape)
        B, Hx, Wx = 50,16,16
        x = self.patch_embed(x)
        nx = x.shape[1]

        deform_inputsz1, deform_inputsz2 = deform_inputs(z)
        cz1, cz2, cz3, cz4 = self.spm(z)
        cz2, cz3, cz4 = self._add_level_embed(cz2, cz3, cz4)
        cz = torch.cat([cz2, cz3, cz4], dim=1)  # B*N*C
        B, Hz, Wz = z.shape[0], 8,8
        ...
        for i, layer in enumerate(self.interactions):
            ...
            # x, cx,z,cz,attn_res = layer(x, cx,z,cz, self.blocks[indexes[0]:indexes[-1] + 1],
            #              deform_inputs1, deform_inputs2,deform_inputsz1, deform_inputsz2, Hx, Wx,Hz,Wz,attn_res)
            #x, cx, z, cz = layer(x, cx, z, cz, self.blocks[indexes[0]:indexes[-1] + 1],
            #                               deform_inputs1, deform_inputs2, deform_inputsz1, deform_inputsz2, Hx, Wx, Hz,
            #                               Wz)
            x = layer(x, cx, z, cz, self.blocks[indexes[0]:indexes[-1] + 1],self.aifi,
                                           deform_inputs1, deform_inputs2, deform_inputsz1, deform_inputsz2, Hx, Wx, Hz,
                                           Wz)                                           
