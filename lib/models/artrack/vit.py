...
import sys
print(sys.path)
from lib.models.artrack.comer_modules import CTIBlock,CNN
...
class AIFI(TransformerEncoderLayer):
    ...
    def forward(self, x):
        ...
        c, h, w = x.shape[2],math.sqrt(x.shape[1]),math.sqrt(x.shape[1])
...
class Attention(nn.Module):
    ...
    '''def forward(self, x, attn_res=None,alpha=0.5,return_attention=False):
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        if return_attention:
            ...
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x
            # x1, attn_res = self.attn(self.norm1(x), attn_res, alpha)
...
class VisionTransformer(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.interaction_indexes = [[0,11]]
        #print(len(self.interaction_indexes),'******89898989898')
        self.init_weights(weight_init)
        self.spm = CNN(inplanes=64,
                       embed_dim=embed_dim)
        use_CTI_toV = [True, True]
        use_CTI_toC = [False, False]
        cnn_feature_interaction = [True, True]
        attn_res = None
        self.level_embed = nn.Parameter(torch.zeros(3, embed_dim))
        self.interactions = nn.Sequential(*[
            CTIBlock(dim=embed_dim, num_heads=12, n_points=4,
                     init_values=0, drop_path=drop_path_rate,
                     ...
                     #extra_CTI=((True if i == len(self.interaction_indexes) - 1 else False)),
                     extra_CTI = False,
                     extra_num=4)
