...
class CTIBlock(nn.Module):
    ...
    #def forward(self, x, cx,z,cz, blocks, deform_inputs1, deform_inputs2, deform_inputsz1, deform_inputsz2, Hx, Wx,Hz,Wz,attn_res = None):
    def forward(self, x, cx,z,cz, blocks,aifi, deform_inputs1, deform_inputs2, deform_inputsz1, deform_inputsz2, Hx, Wx,Hz,Wz):
        B, N, C = x.shape
        deform_inputs = deform_inputs_only_one(x, Hx*16, Wx*16)
        deformz_inputs = deform_inputs_only_one(z, Hz*16, Wz*16)
        if self.use_CTI_toV:
            cx = self.mrfp(cx, Hx, Wx)
            cx_select1, cx_select2, cx_select3 = cx[:,:Hx*Wx*4, :], cx[:, Hx*Wx*4:Hx*Wx*4+Hx*Wx, :], cx[:, Hx*Wx*4+Hx*Wx:, :]
            cx = torch.cat([cx_select1, cx_select2 + x, cx_select3], dim=1)
            
            x = self.cti_tov(query=x, reference_points=deform_inputs[0],
                          feat=cx, spatial_shapes=deform_inputs[1],
                          level_start_index=deform_inputs[2], H=Hx, W=Wx)
            x = aifi(x)
            cz = self.mrfp(cz, Hz, Wz)
            cz_select1, cz_select2, cz_select3 = cz[:, :Hz * Wz * 4, :], cz[:, Hz * Wz * 4:Hz * Wz * 4 + Hz * Wz, :], cz[:, Hz * Wz * 4 + Hz * Wz:,:]
            cz = torch.cat([cz_select1, cz_select2 + z, cz_select3], dim=1)
            
            z = self.cti_tov(query=z, reference_points=deformz_inputs[0],
                             feat=cz, spatial_shapes=deformz_inputs[1],
                             level_start_index=deformz_inputs[2], H=Hz, W=Wz)
            z = aifi(z)
            _,z1,_ = z.shape
            _,x1,_ = x.shape
        ...
        for idx, blk in enumerate(blocks):
            #x, attn_res = blk(x, attn_res, self.alpha)
            x = blk(x)
        #print('self.use_CTI_toC:',self.use_CTI_toC)

        #z,x = torch.split(x,[z1,x1],dim=1)

        '''if self.use_CTI_toC:
            cx = self.cti_toc(query=cx, reference_points=deform_inputs2[0],
                           feat=x, spatial_shapes=deform_inputs2[1],
                           level_start_index=deform_inputs2[2], H=Hx, W=Wx)
            cz = self.cti_toc(query=cz, reference_points=deform_inputsz2[0],
                             feat=z, spatial_shapes=deform_inputsz2[1],
                             level_start_index=deform_inputsz2[2], H=Hz, W=Wz)
                           
        if self.extra_CTIs is not None:
            for cti in self.extra_CTIs:
                cx = cti(query=cx, reference_points=deform_inputs2[0],
                              feat=x, spatial_shapes=deform_inputs2[1],
                              level_start_index=deform_inputs2[2], H=Hx, W=Wx)
                cz = cti(query=cz, reference_points=deform_inputsz2[0],
                        feat=z, spatial_shapes=deform_inputsz2[1],
                        level_start_index=deform_inputsz2[2], H=Hz, W=Wz)'''
        #print("arseqx",x.shape)
        return x
